...
class newsPage:
    ...
    def pageStructure(self):
        ...
        st.header("News")
        filter_options = st.selectbox("Select League", self.df["League"].unique())
        filtered_df = self.df[self.df["League"] == filter_options]
        return filtered_df


    def strucureAricles(self):
        filtered_df = self.pageStructure()
        resized_images = []
        titles = []
        urls = []
        sources = []
    ...
    def writeArticles(self):
        ...
        resized_images, titles, urls, sources = self.strucureAricles()
...
#     df = readData("franchisecricket", "fcDataSets", "newsDF")
